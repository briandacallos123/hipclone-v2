...
const PatientUpcomingAppt = () => {
  ...
  const changeActiveDoctor = useCallback((row: any) => {
  ...
  const NoDataDetails = (
    <Box rowGap={2} sx={{
      ...
      <Button component="a" href={paths.dashboard.appointment.find} variant="contained">Book Now!</Button>
    </Box>
  )
    ...
    <Stack sx={{
      ...
      {upMd && <>
        ...
        <Box sx={{
          ...
          {upcommingResult?.loading ? <UpcomingSkeleton /> :
           !noDataFound &&  <Grid container>
           <Grid sx={{
             position: 'relative'
           }} item lg={12}>
             <Stack sx={{ mb: 1 }} alignItems='center' direction="row" gap={1}>
               ...
               <Typography sx={{
                 color: 'white',
                 fontSize: 12
               }}>{isEndDisable + 1} / {data?.totalRecords}</Typography>
               <IconButtonStyle
             ...
             <Stack gap={2}>
               <Stack direction="row" gap={{ xs: 1, md: 0 }}>
                 ...
                 <Box>
                   ...
                   <Typography sx={{ color: '#d1d1d1' }}>{doctorTitle} | {doctorClinic}</Typography>
                 </Box>
               </Stack>
               ...
               <Stack gap={1}>
                 <Stack direction="row" gap={2}>
                   ...
                   <Typography sx={{ color: 'white' }}>{dateForAppt(activeDoctor?.date)}</Typography>

                 </Stack>
                 <Stack direction="row" gap={2}>
                   <Icon color="white" icon="teenyicons:clock-outline" fontSize={22} />
                   <Typography sx={{ color: 'white' }}>{formatMilitaryTime(activeDoctor?.time_slot)}</Typography>
      ...
      {!upMd && <Box sx={{
        ...
        {upcommingResult?.loading ? <UpcomingSkeleton /> : <PatientUpcommingCarousel isLeftEnd={isLeftEnd} handleMove={handleMove} changeActiveDoctor={changeActiveDoctor} total={data?.totalRecords} onIncrement={onIncrement} activeDoctor={activeDoctor} loading={upcommingResult.loading} data={data?.tableData} />}
      </Box>}
