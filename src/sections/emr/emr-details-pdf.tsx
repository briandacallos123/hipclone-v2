...
export default function EmrDetailsPDF({ item, allData, soapData, prescription }: Props) {
  ...
  const PROFILE_INFO = [
    {
      category: 'personal',
      data: [
        { label: 'Birthdate', value: fDate(item?.patientRelation?.BDAY) || '-' },
        { label: 'Age', value: item?.patientRelation?.AGE || '-' },
        {
          label: 'Gender',
          value: capitalize(item?.patientRelation?.SEX === 1 ? 'MALE' : 'Female') || '-',
        },
        { label: 'Blood type', value: item?.patientRelation?.BLOOD_TYPE || '-' },
      ],
    },
    {
      category: 'contact',
      data: [
        { label: 'Address', value: item?.patientRelation?.HOME_ADD || '-' },
        { label: 'Phone number', value: item?.patientRelation?.CONTACT_NO || '-' },
        { label: 'Email', value: item?.patientRelation?.EMAIL || '-' },
      ],
    },
    {
      category: 'employment',
      data: [
        { label: 'Occupation', value: item?.patientRelation?.OCCUPATION || '-' },
        { label: 'Employer', value: item?.patientRelation?.EMPLOYERSNAME || '-' },
        { label: 'Address', value: item?.patientRelation?.EMPLOYERSADDRESS || '-' },
        { label: 'Phone number', value: item?.patientRelation?.EMPLOYERSPHONENO || '-' },
      ],
    },
    {
      category: 'emergency',
      data: [
        { label: 'Name', value: item?.patientRelation?.EMERGENCYNAME || '-' },
        { label: 'Address', value: item?.patientRelation?.EMERGENCYADDRESS || '-' },
        { label: 'Phone number', value: item?.patientRelation?.EMERGENCYCONTACTNO || '-' },
        { label: 'Relationship', value: item?.patientRelation?.EMERGENCYRELATIONSHIP || '-' },
      ],
    },
    {
      category: 'physician',
      data: [
        { label: 'Referring', value: item?.patientRelation?.REFFERINGPHYSICIAN || '-' },
        { label: 'Primary', value: item?.patientRelation?.PRIMARYCAREPHYSICIAN || '-' },
        { label: 'Other', value: item?.patientRelation?.OTHERPHYSICIAN || '-' },
      ],
    },
  ];
  ...
  return (
    <Document>
      ...
      <View style={[styles.mb8, { justifyContent: 'center', textAlign: 'center' }]}>
          <Text style={styles.h3}>{item?.patientRelation?.FNAME} {item?.patientRelation?.MNAME} {item?.patientRelation?.LNAME}</Text>
          <Text style={styles.body1}>{item?.patientRelation?.CONTACT_NO} | {item?.patientRelation?.EMAIL}</Text>
        </View>
        {renderDetails}
