...
//
import { emr_view_patient_medical_history_data } from '@/libs/gqls/medicalProfile';
...
export default function EmrProfileMedical({ data, clientside, loading: loading1 }: Props) {
  ...
  const {
    ...
    loading,
    refetch,
  }: any = useQuery(emr_view_patient_medical_history_data, {
    context: {
      requestTrackerId:
        'emr_view_patient_medical_history_data[emr_view_medical_history_input_request]',
    },
  ...
  useEffect(() => {
    if (data2) {
      const { emr_view_patient_medical_history_data } = data2;
      setTableData(emr_view_patient_medical_history_data?.emr_view_medical_history_data);
      setTableData1(emr_view_patient_medical_history_data?.view_medical_history_data);
      setTotalRecords(emr_view_patient_medical_history_data?.total_records);
      setAllData((prev: any) => {
        return {
          ...prev,
          medical_history: emr_view_patient_medical_history_data?.view_medical_history_data,
        };
      });
  ...
  const renderDataOrSkeletons = () => {
    if (loading1) {
      ...
      return (
        ...
        >
          <Typography variant="body2">No Medical History available</Typography>
        </div>
      );
    }
  };
  ...
  return (
    <>
      <Card sx={{ height: '100%' }}>
        <CardHeader
          title={<Typography variant={!upMd ? 'subtitle1' : 'h6'}>Medical History</Typography>}
          action={
      ...
      <EmrProfileCreateView
        tableDataMedicalHistory={tableData}
      ...
      >
        <DialogTitle>Medical History Information</DialogTitle>
...
function ItemTableRow({ row }: ItemTableRowProps) {
  ...
  return (
    <TableRow hover>
      <TableCell sx={{ typography: 'subtitle2' }}>{row?.medhistory}</TableCell>
