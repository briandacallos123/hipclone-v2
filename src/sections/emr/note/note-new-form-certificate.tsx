...
import { POST_MED_CERT_EMR } from '@/libs/gqls/notes/noteMedCert';
...
export default function NoteNewFormCertificate({
  onClose,
  data: emrRef,
  refetch: onRefetch,
}: Props) {
  ...
  // console.log('paitetn', emrRef?.patientRelation);
  ...
  const [createMedCert] = useMutation(POST_MED_CERT_EMR);
  const handleSubmitValue = useCallback(
    ...
    [createMedCert, enqueueSnackbar, isLinked, refIdEMR, refIdPatient, reset, snackKey]
  );
          ...
          <Stack spacing={3}>
            ...
            <div>
              <TypographyStyle gutterBottom>
                ...
                <span>
                  {` ${emrRef?.patientRelation?.FNAME} ${emrRef?.patientRelation?.LNAME}, ${emrRef?.patientRelation?.AGE} yr(s) old, `}
                </span>
