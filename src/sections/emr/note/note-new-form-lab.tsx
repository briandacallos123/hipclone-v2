...
import { POST_NOTES_LAB_EMR } from 'src/libs/gqls/notes/notesLabReq';
...
export default function NoteNewFormLaboratory({
  onClose,
  data: emrRef,
  refetch: onRefetch,
}: Props) {
  ...
  // useEffect(() => {
  //   procedureFetch().then((result: any) => {
  //     const { data } = result;
  //     if (data) {
  //       const { QueryAllPayProcedure } = data;
  //       setProcedureData(QueryAllPayProcedure);
  //     }
  //   });
  //   return () => procedureFetch();
  // }, [procedureFetch]);
  ...
  const [createLabReq] = useMutation(POST_NOTES_LAB_EMR);
  const handleSubmitValue = useCallback(
    ...
    [createLabReq, enqueueSnackbar, isLinked, refIdEMR, refIdPatient, reset, snackKey]
  );
