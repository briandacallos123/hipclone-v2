...
import { POST_NOTES_TXT_EMR } from 'src/libs/gqls/notes/notesTxt';
// _mock
...
export default function NoteNewFormText({ onClose, data: emrRef, refetch: onRefetch }: Props) {
  ...
  const values = watch();
  const [createNotesText] = useMutation(POST_NOTES_TXT_EMR);

  const handleSubmitValue = useCallback(
    async (model: NexusGenInputs['PostNotesTxtEMRInputs']) => {
      const data: NexusGenInputs['PostNotesTxtEMRInputs'] = {
    ...
    [createNotesText, enqueueSnackbar, isLinked, refIdEMR, refIdPatient, reset, snackKey]
  );
  ...
  const onSubmit = useCallback(
    async (data: any) => {
      try {
        ...
        // Object.entries(data)?.map(([key, val]) => MyForm.append(key, val));
        onClose();
