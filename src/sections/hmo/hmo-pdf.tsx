...
export default function HmoLoaPDF({ item }: Props) {
  ...
  const patientName = `${item?.patientHmoInfo?.patient?.FNAME} ${item?.patientHmoInfo?.patient?.MNAME} ${item?.patientHmoInfo?.patient?.LNAME}`;
  // SEX
  // AGE
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={[styles.gridContainer, styles.mb8]}>
          <View style={styles.col8}>
            ...
            <Text style={styles.body1}>
              <Text style={styles.h5}>Age : </Text> {item?.patientHmoInfo?.patient?.AGE || 20} yrs old,&nbsp;
            </Text>
            <Text style={styles.body1}>
              <Text style={styles.h5}>Gender: </Text> {(item?.patientHmoInfo?.patient?.SEX === 1 && 'Male') || (item?.patientHmoInfo?.patient?.SEX === 2 && 'Female')}
            </Text>
            <Text style={styles.body1}>
            <Text style={styles.h5}>Address : </Text> {item?.patientHmoInfo?.patient?.HOME_ADD}
            </Text>
          </View>
        ...
        <View style={[styles.stackContainer, styles.mb40]}>
          {item?.appointmentInfo?.appt_hmo_attachment?.map((urlObject: any) => {
          ...
          {/* {item?.appointmentInfo?.appt_hmo_attachment?.map((url:any) => (
