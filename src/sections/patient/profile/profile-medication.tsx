...
//
import { view_patient_medication_data } from '@/libs/gqls/medicalProfile';
...
export default function PatientProfileMedication({ data, clientside, loading1 }: Props) {
  ...
  const [totalRecords, setTotalRecords]: any = useState(null);
  ...
  const {
    ...
    loading,
    refetch,
  }: any = useQuery(view_patient_medication_data, {
    context: {
      requestTrackerId: 'view_patient_medication_data[view_medication_input_request]',
    },
  ...
  // useEffect(() => {
  //   console.log(data2, 'data2data2data2data2data2data2data2data2data2data2data2data2data2');

  //   if (data2) {
  //     if (typeof totalRecords === 'number' && Number(totalRecords) === 0 && !tableData1?.length) {
  //       console.log('Awittt@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@');
  //       setNotFound1(true);
  //     }
  //   }
  ...
  useEffect(() => {
    if (data2) {
      // console.log(data2, 'data2data2data2data2data2data2data2data2data2data2data2data2data2');
      const { view_patient_medication_data } = data2;
      setTableData(view_patient_medication_data?.view_medication_data);
      setTableData1(view_patient_medication_data?.medication_data);
      setTableLoading(false);
      setTotalRecords(view_patient_medication_data?.total_records);
      setAllData((prev: any) => {
        return { ...prev, medication: view_patient_medication_data?.medication_data };
      });
    }
  }, [data2]);
  ////////////////////////////////////////////////////////////////////////////
  ...
  return (
    <>
      <Card sx={{ height: '100%' }}>
        <CardHeader
          title={<Typography variant={!upMd ? 'subtitle1' : 'h6'}>Medication</Typography>}
          action={
        ...
        <Stack sx={{ pt: { md: 0, xs: 0.2 }, px: { md: 2, xs: 1.5 }, pb: { md: 2, xs: 0.5 } }}>
          ...
          {notFound && nodatatext()}
          {/* {!tableData1 ? (
            [...Array(5)].map((_) => <Skeleton key={_} variant="text" />)
          ) : (
            <>
      ...
      <PatientProfileCreateView
        ...
        category="Medication"
        refetch={refetch}
        isLoading4={isLoading}
        setLoading4={setLoading}
      />
...
function ItemTableRow({ row }: ItemTableRowProps) {
  ...
  return (
    <TableRow hover>
      <TableCell sx={{ typography: 'subtitle2' }}>{row?.medication}</TableCell>
