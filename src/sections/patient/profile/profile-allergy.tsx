...
//
import { view_patient_allegy_data } from '@/libs/gqls/medicalProfile';
...
export default function PatientProfileAllergy({ data, clientside, loading1: loading1 }: Props) {
  ...
  const {
    ...
    loading,
    refetch,
  }: any = useQuery(view_patient_allegy_data, {
    context: {
      requestTrackerId: 'view_patient_allegy_data[view_allergy_input_request]',
    },
  ...
  useEffect(() => {
    if (data2) {
      const { view_patient_allegy_data } = data2;
      setTableData(view_patient_allegy_data?.view_allegy_data);
      setTableData1(view_patient_allegy_data?.allegy_data);
      setTotalRecords(view_patient_allegy_data?.total_records);
      setAllData((prev: any) => {
        return { ...prev, allergies: view_patient_allegy_data?.allegy_data };
      });
  ...
  const nodatatext = () => {
    return (
      ...
      >
        ...
        <Typography  sx={{
        mb:1
      }} variant="body2">No allergies available</Typography>
