...
//
import { view_patient_smoking_data } from '@/libs/gqls/medicalProfile';
...
export default function PatientProfileSmoking({ data, clientside, loading1: loading1 }: Props) {
  ...
  const {
    ...
    loading,
    refetch,
  }: any = useQuery(view_patient_smoking_data, {
    context: {
  ...
  useEffect(() => {
    if (data2) {
      const { view_patient_smoking_data } = data2;
      setTableData(view_patient_smoking_data?.view_smoking_data);
      setTableLoading(false);
      setTableData1(view_patient_smoking_data?.smoking_data);
      setTotalRecords(view_patient_smoking_data?.total_records);
      setAllData((prev: any) => {
        return { ...prev, smoking: view_patient_smoking_data?.smoking_data };
      });
