...
export default function PatientCarouselSearch({ item }: Props) {
  ...
  useEffect(() => {
    if (filterName.length >= 3) {
      setTableData(
        item?.filter(
          (data: any) =>
            data?.patientInfo?.FNAME.toLowerCase().indexOf(filterName.toLowerCase()) !== -1 ||
            data?.patientInfo?.FNAME.toLowerCase().indexOf(filterName.toLowerCase()) !== -1
        )
      );
    }
      ...
      >
        <Stack spacing={1}>
          ...
          {tableData
            ...
            .map((data: any) => (
              <CarouselItem key={data.S_ID} item={data} />
            ))}
        </Stack>
      </CustomPopover>
    </>
...
function CarouselItem({ item, sx }: CarouselItemProps) {
  ...
  const info = `${item.patientInfo?.SEX === 1 ? 'MALE' : 'FEMALE'}, ${
