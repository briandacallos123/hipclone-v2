...
const QueueController = () => {
  ...
  console.log(notToday,'NOT TODAY_________')
  ...
  useEffect(() => {
    if (queryData) {
      ...
      // console.log(queryData,'HAHAHA__________HAHAHAHA__________HAHAHAHA')
      ...
      if (queryData?.QueuePatient && queryData?.QueuePatient?.position !== -1) {
        ...
        if(queryData?.QueuePatient?.is_not_today){
          ...
          // setToday(false)
          // setDataToday(queryData?.QueuePatient?.appointments_data[0])
        }
  ...
  return { QueryQueue, data, dataResults, refetch, loading, position, remainingP, newPosition, notToday }
}
