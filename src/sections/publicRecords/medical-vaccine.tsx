...
import NotePDFVaccine from '../note/note-pdf-vaccine';
...
const MedicalVaccine = () => {
  ...
  const [getMedical, { data, loading, error }] = useLazyQuery(get_note_vaccine, {
    context: {
    ...
    if (id) {
      getMedical({
        ...
        if (data) {
          ...
          setCurrentItem(QueryNotesPedCertObj);
  ...
  useEffect(() => {
    (async () => {
      const link = `http://localhost:9092/records/medical-vaccine/${currentItem?.qrcode}`
      // setLink(link)
              ...
              <Grid item sm={12} md={12} lg={12}>
                {currentItem && isOpen && <Box sx={{ height: '100%', overflow: 'hidden' }}>
                  <PDFViewer width="100%" height="100%" style={{ border: 'none' }}>
                    ...
                    <NotePDFVaccine  qrImage={qrImage} item={currentItem}/>
                  </PDFViewer>
                </Box>}
              </Grid>
            </Grid>
